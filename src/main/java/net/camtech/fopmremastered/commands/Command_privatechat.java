	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length == 1) {
			...
			if (args[0].equalsIgnoreCase("off")) {
				...
				FreedomOpModRemastered.configs.getAdmins().getConfig().set(player.getUniqueId().toString() + ".chat",
						"");
				return true;
			}
		...
		if (args.length == 2) {
			...
			if (args[0].equalsIgnoreCase("delete")) {
				if (!FOPMR_PrivateChats.removeChat(player, args[1])) {
					sender.sendMessage(
							ChatColor.RED + "Could not remove chat, you are not the owner or the chat does not exist.");
					return true;
				}
		...
		if (args.length == 3) {
			...
			if (args[0].equalsIgnoreCase("colour") || args[0].equalsIgnoreCase("color")) {
				...
				if (!FOPMR_PrivateChats.changeColour(player, args[1], colour)) {
					sender.sendMessage(
							ChatColor.RED + "You are not the owner of this chat or the chat does not exist.");
					return true;
				}
			...
			if (args[0].equalsIgnoreCase("add")) {
				if (!FOPMR_PrivateChats.addPlayer(player, player2, args[1])) {
					sender.sendMessage(ChatColor.RED
							+ "You are not the owner of the chat, the chat does not exist or the player is already in the chat.");
					return true;
				}
			...
			if (args[0].equalsIgnoreCase("remove")) {
				if (!FOPMR_PrivateChats.removePlayer(player, player2, args[1])) {
					sender.sendMessage(ChatColor.RED
							+ "You are not the owner of the chat (and the selected player is not you), the chat does not exist or the player is not in the chat.");
					return true;
				}
