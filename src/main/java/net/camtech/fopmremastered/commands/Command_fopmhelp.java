					...
					if (matcher.find()) {
						try {
							Class<?> commandClass = Class
									.forName("net.camtech.fopmremastered.commands." + matcher.group(1));
							...
							if (commandClass.isAnnotationPresent(CommandParameters.class)) {
								...
								cmdconstructed = new FOPMR_BlankCommand(params.name(), params.usage(),
										params.description(), Arrays.asList(params.aliases().split(", ")),
										params.rank(), commandClass);
							} else {
								Constructor construct = commandClass.getConstructor();
								cmdconstructed = (FOPMR_Command) construct.newInstance();
							}
							String message = ChatColor.GOLD + cmdconstructed.command + ChatColor.GREEN + " :|: "
									+ ChatColor.BLUE + cmdconstructed.description + ChatColor.GREEN + " :|: "
									+ ChatColor.AQUA + cmdconstructed.usage;
							if (!(cmdconstructed.alias == null) && !cmdconstructed.alias.isEmpty()) {
								message = message + ChatColor.GREEN + " :|: " + ChatColor.RED + "["
										+ StringUtils.join(cmdconstructed.alias, ", ") + "]";
							}
