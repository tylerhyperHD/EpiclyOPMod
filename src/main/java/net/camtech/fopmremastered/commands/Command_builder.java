	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length == 1) {
			...
			if (args[0].equalsIgnoreCase("list")) {
				...
				FreedomOpModRemastered.configs.getAdmins().getConfig().getKeys(false).stream().filter((
						uuid) -> (FreedomOpModRemastered.configs.getAdmins().getConfig().getBoolean(uuid + ".builder")))
						.forEach((uuid) -> {
							sender.sendMessage(ChatColor.GREEN + " - " + FreedomOpModRemastered.configs.getAdmins()
									.getConfig().getString(uuid + ".lastName"));
						});
				return true;
			}
		}
		if (args.length == 2) {
			...
			if (args[0].equalsIgnoreCase("add")) {
				FOPMR_Commons.adminAction(sender.getName(), "Adding " + player.getName() + " to Master Builder.",
						false);
			...
			if (args[0].equalsIgnoreCase("remove")) {
				FOPMR_Commons.adminAction(sender.getName(), "Removing " + player.getName() + " from Master Builder.",
						true);
