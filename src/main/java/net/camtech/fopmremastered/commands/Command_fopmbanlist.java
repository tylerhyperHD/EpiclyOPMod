...
public class Command_fopmbanlist {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		for (String name : FreedomOpModRemastered.configs.getBans().getConfig().getConfigurationSection("names")
				.getKeys(false)) {
		...
		for (String ip : FreedomOpModRemastered.configs.getBans().getConfig().getConfigurationSection("ips")
				.getKeys(false)) {
		...
		for (String uuid : FreedomOpModRemastered.configs.getBans().getConfig().getConfigurationSection("uuids")
				.getKeys(false)) {
		...
		if (args.length == 0) {
			...
			if ("clear".equalsIgnoreCase(args[0])) {
				...
				case "names":
					message = "Name";
					for (String name : FreedomOpModRemastered.configs.getBans().getConfig()
							.getConfigurationSection("names").getKeys(false)) {
						if (FreedomOpModRemastered.configs.getBans().getConfig()
								.getBoolean("names." + name + ".perm")) {
							continue;
						}
				...
				case "ips":
					...
					for (String ip : FreedomOpModRemastered.configs.getBans().getConfig().getConfigurationSection("ips")
							.getKeys(false)) {
						if (FreedomOpModRemastered.configs.getBans().getConfig()
								.getBoolean("ips." + ip.replaceAll("\\.", "-") + ".perm")) {
							continue;
						}
						FreedomOpModRemastered.configs.getBans().getConfig().set("ips." + ip.replaceAll("\\.", "-"),
								null);
						FreedomOpModRemastered.configs.getBans().saveConfig();
					}
					break;
				case "uuids":
					message = "UUID";
					for (String uuid : FreedomOpModRemastered.configs.getBans().getConfig()
							.getConfigurationSection("uuids").getKeys(false)) {
						if (FreedomOpModRemastered.configs.getBans().getConfig()
								.getBoolean("uuids." + uuid + ".perm")) {
							continue;
						}
				...
				sender.sendMessage(
						ChatColor.AQUA + message + ChatColor.GREEN + " banlist has been cleared successfully.");
				return true;
			}
			return false;
		}
