		...
		if (args.length == 1) {
			if (args[0].equalsIgnoreCase("list")) {
				...
				for (String name : admins.getConfigurationSection("").getKeys(false)) {
					if (!admins.getString(name + ".rank").equalsIgnoreCase("op")) {
						adminlist.put(
								CUtils_Methods.colour(" " + ChatColor.GOLD + admins.getString(name + ".lastName")),
								admins.getString(name + ".rank"));
					}
				}
				...
				ArrayList<String> eomcreators = new ArrayList<>();
				...
				sender.sendMessage(ChatColor.GOLD + "    - The EOM Creator:");
				sender.sendMessage("        - " + StringUtils.join(eomcreators, ", "));
			...
			if (args[0].equalsIgnoreCase("purge") && FOPMR_Rank.isSenior(sender)) {
				...
				for (String name : admins.getConfigurationSection("").getKeys(false)) {
					...
					if (change > 604800000) {
						sender.sendMessage("Removed " + admins.getString(name + ".lastName")
								+ ", time since last login in milliseconds: " + change + ".");
				...
				sender.sendMessage(
						ChatColor.GOLD + "Admin Name " + ChatColor.RED + " :|: " + ChatColor.GOLD + "Last Login Date.");
				for (String name : removedAdmins.keySet()) {
					Date date = new Date(removedAdmins.get(name));
					sender.sendMessage(
							ChatColor.GOLD + name + ChatColor.RED + " :|: " + ChatColor.GOLD + date.toGMTString());
				}
				return true;
			}
			return false;
		...
		if (args.length >= 3) {
			...
			if (level == 0) {
				Bukkit.broadcastMessage(
						StringUtils.join(ArrayUtils.subarray(args, 2, args.length), " ") + " is an invalid rank.");
				return true;
			}
			if (level == 6 || level == 7) {
