...
public class Command_commandblock {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		String message = StringUtils.join(ArrayUtils.subarray(args, 3, args.length), " ", 0,
				ArrayUtils.subarray(args, 3, args.length).length);
		FreedomOpModRemastered.configs.getCommands().getConfig().set(args[0].toLowerCase() + ".rank", rank);
		FreedomOpModRemastered.configs.getCommands().getConfig().set(args[0].toLowerCase() + ".kick",
		...
		sender.sendMessage(ChatColor.RED + "Blocking command " + args[0].toLowerCase() + " to clearance level " + rank
				+ " or above with the message \"" + message + "\". This command will " + not + "kick the user.");
		return true;
	}
}
