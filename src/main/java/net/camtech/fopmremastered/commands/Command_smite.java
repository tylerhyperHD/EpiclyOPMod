...
public class Command_smite {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (player.getName().equals("PiggoWink") && player.getName().equals("_herobrian35_")
				&& player.getName().equals("Herobrine105")) {
	...
	public void smite(Player player, String reason) {
		Bukkit.broadcastMessage(
				String.format(ChatColor.GOLD + "%s has been a naughty, naughty person.\nThey have thus been smitten!\n"
		...
		if (Bukkit.getPluginManager().isPluginEnabled("BarAPI")) {
			BarAPI.setMessage((full.length() <= 64 ? full : String.format("%s has been smitten!", player.getName())),
					10);
		}
		...
		for (int x = -1; x <= 1; x++) {
			for (int z = -1; z <= 1; z++) {
				final Location strike_pos = new Location(world, targetPos.getBlockX() + x, targetPos.getBlockY(),
