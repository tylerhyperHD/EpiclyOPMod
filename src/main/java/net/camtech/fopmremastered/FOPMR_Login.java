...
public class FOPMR_Login {
	...
	public static final List<String> permbannedNames = Arrays.asList("Jellow_", "KingSquads", "ButterWarrior146",
			"FGL_Karma", "Mahl_Trollbait", "TheRealTrioligy");
	public static final List<String> permbannedIps = Arrays.asList("70.162.*.*", "66.168.*.*", "86.100.*.*",
			"120.29.*.*", "50.35.*.*");
	public static final List<String> permbannedUuids = Arrays.asList("42711b44-eccc-3115-8a17-4447f5a4febd",
			"31dcd227-9c62-4b6a-98b5-47ee6761ef74", "69a38028-1573-494e-b2fb-dca156233649",
			"18d313cb-c5cc-43c7-8838-8a32cce9c987", "1033096e-3c1e-494d-bc6b-84d2d5f64c82",
	...
	public static void handleJoinEvent(PlayerJoinEvent event) {
		...
		if (Bukkit.getPluginManager().getPlugin("TitleAPI") != null) {
			TitleAPI.sendTitle(player, 20, 40, 20,
					CUtils_Methods.colour("&-Hi there " + CUtils_Methods.randomChatColour() + player.getName() + "&-!"),
					CUtils_Methods.colour("&-Welcome to " + CUtils_Methods.randomChatColour()
							+ FreedomOpModRemastered.plugin.getConfig().getString("general.name") + "&-!"));
			TitleAPI.sendTabTitle(player,
					CUtils_Methods.colour(
							"&-Welcome to EpicFreedom " + CUtils_Methods.randomChatColour() + player.getName() + "&-!"),
					CUtils_Methods.colour("&-Running the " + CUtils_Methods.randomChatColour()
							+ "FreedomOpMod: Remastered &-by Camzie99!"));
		}
		...
		if (config.contains(player.getUniqueId().toString())) {
			...
			if (!(config.getString(player.getUniqueId().toString()) + ".lastIp")
					.equals(player.getAddress().getHostString()) && FOPMR_Rank.getRank(player) == FOPMR_Rank.Rank.OP) {
			...
			if (player.getName().equals("tylerhyperHD")) {
				if (FOPMR_Rank.isImposter(player)) {
					...
					event.setJoinMessage(
							ChatColor.AQUA + "" + "Tyler Hyper" + " is the " + ChatColor.BLUE + "EOM-Creator");
				}
			} else if (!"default".equals(config.getString(player.getUniqueId().toString() + ".login"))) {
				event.setJoinMessage(ChatColor.AQUA + player.getName() + " "
						+ CUtils_Methods.colour(config.getString(player.getUniqueId().toString() + ".login")));
			} else if (FOPMR_Rank.getRank(player) != FOPMR_Rank.Rank.OP) {
				event.setJoinMessage(ChatColor.AQUA + player.getName() + " is "
						+ CUtils_Methods.aOrAn(FOPMR_Rank.getRank(player).name) + " "
						+ FOPMR_Rank.getRank(player).name);
			}
			if (FOPMR_Rank.getRank(player) == Rank.IMPOSTER) {
				Bukkit.broadcastMessage(ChatColor.RED + player.getName() + " is an imposter!");
				player.sendMessage(
						ChatColor.RED + "Please verify you are who you are logged in as or you will be banned!");
			}
		} else {
			player.sendMessage(ChatColor.GREEN + "Hey there! Welcome to "
					+ FreedomOpModRemastered.configs.getMainConfig().getConfig().getString("general.name") + "!");
			config.set(player.getUniqueId().toString() + ".lastName", player.getName());
			config.set(player.getUniqueId().toString() + ".lastIp", player.getAddress().getHostString());
		...
		if (FreedomOpModRemastered.configs.getMainConfig().getConfig().getInt("general.accessLevel") > 0) {
			new BukkitRunnable() {
				@Override
				public void run() {
					player.sendMessage(ChatColor.RED + "Server is currently locked down to clearance level "
							+ FreedomOpModRemastered.configs.getMainConfig().getConfig().getInt("general.accessLevel")
							+ " (" + FOPMR_Rank.getFromLevel(FreedomOpModRemastered.configs.getMainConfig().getConfig()
									.getInt("general.accessLevel")).name
							+ ").");
				}
			}.runTaskLater(FreedomOpModRemastered.plugin, 20L * 5L);
		}
		player.sendMessage(CUtils_Methods.colour(FreedomOpModRemastered.configs.getMainConfig().getConfig()
				.getString("general.joinMessage").replaceAll("%player%", player.getName())));
		config.set(player.getUniqueId().toString() + ".lastLogin", System.currentTimeMillis());
	...
	public static void handleLogin(PlayerLoginEvent event) {
		...
		String uuidhardcodebyebye = "Your UUID has been hardcoded to a permban list. You may not come back to "
				+ FreedomOpModRemastered.configs.getMainConfig().getConfig().getString("general.name") + " ever again.";
		String iphardcodebyebye = "Your ip has been hardcoded to a permban list. You may not come back to "
				+ FreedomOpModRemastered.configs.getMainConfig().getConfig().getString("general.name") + " ever again.";
		String hardcodebyebye = "Your username has been hardcoded to a permban list. You may not come back to "
				+ FreedomOpModRemastered.configs.getMainConfig().getConfig().getString("general.name") + " ever again.";

		for (Player player2 : Bukkit.getOnlinePlayers()) {
			if ((player.getName() == null ? player2.getName() == null : player.getName().equals(player2.getName()))
					&& FOPMR_Rank.isAdmin(player2)) {
		...
		if (hasNonAlpha) {
			event.disallow(PlayerLoginEvent.Result.KICK_OTHER,
					"Your name contains invalid characters, please login using a fully alphanumeric name.");
			event.setResult(PlayerLoginEvent.Result.KICK_OTHER);
		}
		if (FOPMR_Rank.getRank(player).level < mainconfig.getInt("general.accessLevel")) {
			event.disallow(PlayerLoginEvent.Result.KICK_OTHER, "The server is currently locked down to clearance level "
					+ mainconfig.getInt("general.accessLevel") + ".");
			event.setResult(PlayerLoginEvent.Result.KICK_OTHER);
			return;
		}
		if (FOPMR_Rank.isAdmin(player) && !adminconfig.getBoolean(pId + ".imposter")
				&& (adminconfig.getString(pId + ".lastIp").equals(event.getAddress().getHostAddress()))) {
			event.allow();
			return;
		}
		...
		if (FOPMR_Bans.isBanned(FOPMR_PlayerUtility.getName(player), event.getAddress().getHostAddress())) {
			event.disallow(PlayerLoginEvent.Result.KICK_BANNED,
					FOPMR_Bans.getReason(FOPMR_PlayerUtility.getName(player))
							+ "(You may appeal the ban at our forums accessible from "
							+ mainconfig.getString("general.url") + ")");
		...
		if (FOPMR_PlayerUtility.getName(player).equalsIgnoreCase("ZexyZek")) {
			event.disallow(PlayerLoginEvent.Result.KICK_OTHER,
					ChatColor.RED + "You may not troll on this server. Use a different name and do not troll.");
			return;
		}
		if (FOPMR_PlayerUtility.getName(player).equalsIgnoreCase("null")) {
			event.disallow(PlayerLoginEvent.Result.KICK_OTHER,
					ChatColor.RED + "You may not login with a username of null. Come back with a different one.");
			return;
		}
		// Removes venom from ever coming back to the server
		if (FOPMR_PlayerUtility.getName(player).equalsIgnoreCase("Venom_nV")) {
			event.disallow(PlayerLoginEvent.Result.KICK_OTHER,
					ChatColor.RED + "Venom_nV, go away and don't ever come back to this server.");
			return;
		}
