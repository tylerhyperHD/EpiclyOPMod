...
public class FOPMR_Bans {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				player.getWorld().createExplosion(player.getLocation().getX(), player.getLocation().getY(),
						player.getLocation().getZ(), 10f, false, false);
	...
	public static void addBan(String name, String reason, String banner) {
		...
		if (FreedomOpModRemastered.configs.getBans().getConfig().contains(
				"ips." + FreedomOpModRemastered.configs.getAdmins().getConfig().getString(name + ".lastIp"))) {
			ipBan = true;
		}

		if (FreedomOpModRemastered.configs.getBans().getConfig()
				.contains("uuids." + Bukkit.getOfflinePlayer(name).getUniqueId().toString())) {
			uuidBan = true;
		}
		...
		if (!ipBan) {
			FreedomOpModRemastered.configs.getBans().getConfig()
					.set("ips." + FreedomOpModRemastered.configs.getAdmins().getConfig()
							.getString(Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".lastIp")
							.replaceAll("\\.", "-") + ".reason", reason);
			FreedomOpModRemastered.configs.getBans().getConfig()
					.set("ips." + FreedomOpModRemastered.configs.getAdmins().getConfig()
							.getString(Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".lastIp")
							.replaceAll("\\.", "-") + ".perm", false);
		}

		if (!uuidBan) {
			FreedomOpModRemastered.configs.getBans().getConfig()
					.set("uuids." + Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".reason", reason);
			FreedomOpModRemastered.configs.getBans().getConfig()
					.set("uuids." + Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".perm", false);
		}
	...
	public static void unBan(String name) {
		...
		if (FreedomOpModRemastered.configs.getBans().getConfig()
				.contains("ips." + FreedomOpModRemastered.configs.getAdmins().getConfig()
						.getString(Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".lastIp")
						.replaceAll("\\.", "-"))) {
			ipBan = true;
		}
		...
		if (ipBan) {
			if (FreedomOpModRemastered.configs.getBans().getConfig()
					.getBoolean("ips." + FreedomOpModRemastered.configs.getAdmins().getConfig()
							.getString(Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".lastIp")
							.replaceAll("\\.", "-") + ".perm")) {
				Bukkit.broadcastMessage(ChatColor.RED + FreedomOpModRemastered.configs.getAdmins().getConfig()
						.getString(Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".lastIp")
						.replaceAll("\\.", "-") + " is eternally banned.");
			} else {
				FreedomOpModRemastered.configs.getBans().getConfig()
						.set("ips." + FreedomOpModRemastered.configs.getAdmins().getConfig()
								.getString(Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".lastIp")
		...
		if (uuidBan) {
			if (FreedomOpModRemastered.configs.getBans().getConfig()
					.getBoolean("uuids." + Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".perm")) {
				Bukkit.broadcastMessage(ChatColor.RED + Bukkit.getOfflinePlayer(name).getUniqueId().toString()
						+ " is eternally banned.");
			} else {
				FreedomOpModRemastered.configs.getBans().getConfig()
						.set("uuids." + Bukkit.getOfflinePlayer(name).getUniqueId().toString(), null);
			}
		}
		nameBan = false;
		ipBan = false;
	...
	public static boolean isBanned(Player player) {
		...
		return FreedomOpModRemastered.configs.getBans().getConfig()
				.contains("uuids." + player.getUniqueId().toString());
	...
	public static String getReason(String name, String ip) {
		...
		if (FreedomOpModRemastered.configs.getBans().getConfig().contains("ips." + ip.replaceAll("\\.", "-"))) {
			return FreedomOpModRemastered.configs.getBans().getConfig()
					.getString("ips." + ip.replaceAll("\\.", "-") + ".reason");
		}
		if (FreedomOpModRemastered.configs.getBans().getConfig().contains("names." + name)) {
		...
		if (FreedomOpModRemastered.configs.getBans().getConfig()
				.contains("ips." + FreedomOpModRemastered.configs.getAdmins().getConfig()
						...
						.replaceAll("\\.", "-"))) {
			return FreedomOpModRemastered.configs.getBans().getConfig()
					.getString("ips." + FreedomOpModRemastered.configs.getAdmins().getConfig()
							.getString(Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".lastIp")
							.replaceAll("\\.", "-") + ".reason");
		}
		if (FreedomOpModRemastered.configs.getBans().getConfig()
				.contains("uuids." + Bukkit.getOfflinePlayer(name).getUniqueId().toString())) {
			return FreedomOpModRemastered.configs.getBans().getConfig()
					.getString("uuids." + Bukkit.getOfflinePlayer(name).getUniqueId().toString() + ".reason");
		}
		if (name.contains("PvP")) {
