...
public class FOPMR_WorldManager {
	...
	public static void loadWorldsFromConfig() {
		for (String worldName : FreedomOpModRemastered.configs.getWorlds().getConfig().getKeys(false)) {
			...
			if (FreedomOpModRemastered.configs.getWorlds().getConfig().getString(worldName + ".generator")
					.equalsIgnoreCase("flat")) {
				createNewWorld(worldName, new FOPMR_FlatGenerator(), FOPMR_Rank.getFromName(
						FreedomOpModRemastered.configs.getWorlds().getConfig().getString(worldName + ".rank")));
			} else if (FreedomOpModRemastered.configs.getWorlds().getConfig().getString(worldName + ".generator")
					.equalsIgnoreCase("default")) {
				createNewWorld(worldName, FOPMR_Rank.getFromName(
						FreedomOpModRemastered.configs.getWorlds().getConfig().getString(worldName + ".rank")));
			} else if (FreedomOpModRemastered.configs.getWorlds().getConfig().getString(worldName + ".generator")
					.equalsIgnoreCase("empty")) {
				createNewWorld(worldName, new FOPMR_EmptyGenerator(), FOPMR_Rank.getFromName(
						FreedomOpModRemastered.configs.getWorlds().getConfig().getString(worldName + ".rank")));
			} else if (FreedomOpModRemastered.configs.getWorlds().getConfig().getString(worldName + ".generator")
					.equalsIgnoreCase("rollinghills")) {
				createNewWorld(worldName, new FOPMR_RollinghillsGenerator(), FOPMR_Rank.getFromName(
						FreedomOpModRemastered.configs.getWorlds().getConfig().getString(worldName + ".rank")));
			} else {
				Bukkit.broadcastMessage(ChatColor.RED + "The world: " + worldName
	...
	public static void addGuest(String worldname, Player guest, Player moderator) {
		...
		moderator.sendMessage(
				ChatColor.GREEN + "You have added " + guest.getName() + " to the " + worldname + " guest list.");
	...
	public static void removeGuest(String worldname, Player guest, Player moderator) {
		...
		moderator.sendMessage(
				ChatColor.RED + "You have removed " + guest.getName() + " from the " + worldname + " guest list.");
	}
	...
	// Inject code to allow more commands to be issued
	...
	public static void wipeMainWorld() {
		...
		new BukkitRunnable() {
			@Override
			public void run() {
				CUtils_Methods.deleteWorld(world.getWorldFolder());
				Bukkit.getServer().setWhitelist(false);
			}
		}.runTaskLater(FreedomOpModRemastered.plugin, 20L * 5L);
		createNewWorld("flatlands", new FOPMR_FlatGenerator(), Rank.OP);
