...
public class FOPMR_PrivateChat {
	...
	public void sendToChat(Player player, String message) {
		...
		Bukkit.getOnlinePlayers().stream().filter((admin) -> (FOPMR_Rank.getRank(admin).level > this.rank.level
				&& !(admin.getName().equals(this.owner)) && !this.canAccess(admin))).forEach((Player admin) -> {
					admin.sendMessage(colour + "[" + FOPMR_PrivateChat.this.name + " **SPY**] " + player.getName()
							+ ": " + message);
				});
	}
	...
	public boolean addPlayer(Player sender, Player player) {
		...
		FOPMR_PrivateChats.config.saveConfig();
		player.sendMessage(ChatColor.GREEN + "You have been added to the private chat " + this.name + " by "
				+ sender.getName() + ". You can access this chat by typing /pchat " + this.name
				+ " or you can leave by typing /pchat remove " + this.name + " " + player.getName() + ".");
		return true;
	}

	public boolean removePlayer(Player sender, Player player) {
		if ((!isOwner(sender) && !sender.getName().equals(player.getName()))
				|| !this.allowed.contains(player.getName())) {
			return false;
		}
