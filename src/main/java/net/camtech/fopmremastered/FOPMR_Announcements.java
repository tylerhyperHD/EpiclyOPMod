...
public class FOPMR_Announcements {
	...
	public static void setup() {
		...
		FreedomOpModRemastered.configs.getAnnouncements().getConfig().getKeys(false).stream().forEach((announce) -> {
			announce(FreedomOpModRemastered.configs.getAnnouncements().getConfig().getString(announce + ".message"),
					FreedomOpModRemastered.configs.getAnnouncements().getConfig().getInt(announce + ".time"));
		});
	}
